...
const createVendor = async (req, res) => {
    ...
    const { name, contact_name, phone, email, address, about, profile_image_url, services, portfolio } = data;
    ...
    try {
        ...
        const vendorQuery = `
            INSERT INTO vendors (name, contact_name, phone, email, address, about, profile_image_url)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
            RETURNING vendor_id;
        `;
        const vendorResult = await client.query(vendorQuery, [name, contact_name, phone, email, address, about, profile_image_url]);
 ...
 * @route POST /api/v1/vendors/update (Example)
 */
const updateVendor = async (req, res) => {
    const { identifier, data } = req.body;
    const responseIdentifier = identifier || { request_id: null, user_id: null };

    // vendor_id is required to know WHICH vendor to update
    const { 
        vendor_id, 
        name, 
        contact_name, 
    ...
    if (!vendor_id) {
        return res.status(400).json({ 
            identifier: responseIdentifier, 
            data: { error: 'A "vendor_id" is required to update a vendor.' } 
        });
    }
    ...
    try {
        ...
        // 1. Update main 'vendors' table (Dynamically)
        // We build a query to only update fields that were actually sent
        const vendorFields = [];
        const vendorValues = [];
        let paramIndex = 1;

        if (name !== undefined) {
            vendorFields.push(`name = $${paramIndex++}`);
            vendorValues.push(name);
        }
        if (contact_name !== undefined) {
            vendorFields.push(`contact_name = $${paramIndex++}`);
            vendorValues.push(contact_name);
        }
        if (phone !== undefined) {
            vendorFields.push(`phone = $${paramIndex++}`);
            vendorValues.push(phone);
        }
        if (email !== undefined) {
            vendorFields.push(`email = $${paramIndex++}`);
            vendorValues.push(email);
        }
        if (address !== undefined) {
            vendorFields.push(`address = $${paramIndex++}`);
            vendorValues.push(address);
        }
        if (about !== undefined) {
            vendorFields.push(`about = $${paramIndex++}`);
            vendorValues.push(about);
        }
        if (profile_image_url !== undefined) {
            vendorFields.push(`profile_image_url = $${paramIndex++}`);
            vendorValues.push(profile_image_url);
        }

        // Only run the update query if there's something to update
        if (vendorFields.length > 0) {
            vendorValues.push(vendor_id); // Add vendor_id for the WHERE clause
            const updateVendorQuery = `
                ...
                RETURNING *;
            `;
            const result = await client.query(updateVendorQuery, vendorValues);
            if (result.rows.length === 0) {
                 throw new Error('Vendor not found.'); // This will trigger the ROLLBACK
            }
        }

        // 2. Update 'vendor_services' (Destructive & Re-create)
        // If 'services' array is provided, we replace ALL existing services.
        // If 'services' is undefined, we don't touch them.
        // If 'services' is [], we delete all.
        if (services !== undefined) {
            // Delete all existing services for this vendor
            await client.query('DELETE FROM vendor_services WHERE vendor_id = $1', [vendor_id]);
            
            // Insert the new ones
            if (services.length > 0) {
                const serviceQuery = 'INSERT INTO vendor_services (vendor_id, service_id) VALUES ($1, $2)';
                for (const serviceId of services) {
                    await client.query(serviceQuery, [vendor_id, serviceId]);
                }
            }
        }

        // 3. Update 'vendor_portfolio_images' (Destructive & Re-create)
        // Same logic as services.
        if (portfolio !== undefined) {
            // Delete all existing images
            await client.query('DELETE FROM vendor_portfolio_images WHERE vendor_id = $1', [vendor_id]);

            // Insert the new ones
            if (portfolio.length > 0) {
                const portfolioQuery = 'INSERT INTO vendor_portfolio_images (vendor_id, image_url) VALUES ($1, $2)';
                for (const imageUrl of portfolio) {
                    await client.query(portfolioQuery, [vendor_id, imageUrl]);
                }
            }
        }
        ...
        res.status(200).json({ 
            identifier: responseIdentifier, 
            data: { message: "Vendor updated successfully", vendor_id: vendor_id }
        });
